...
func (b *WemixClient) Init() *WemixClient {
	//获取连接与eth客户端
...
// GetNowBlock  获取最新板块
func (b *WemixClient) GetNowBlock() (block int64, err error) {
	var _block string
...
func (b *WemixClient) GetBlockByNumber(blockNumber int64) (block interface{}, err error) {
...
func (b *WemixClient) GetTransferStatus(hash string) (transferData interface{}, err error) {
...
func (b *WemixClient) GetTransactionCount(address string) (nonce interface{}, err error) {
...
//WemixTransferWemix 转账WEMIX
func WemixTransferWemix(privateKeys string, amount *big.Int, toAddress string, MaxNonce uint64) (transferData interface{}, currentNonce uint64, err error) {
	cache := service.Cache.New()
	rpcUrl := gconv.String(cache.Get("wemix_rpc_url"))
	...
	chainID := g.Cfg().GetInt64("wemix.chain_id")
...
//WemixTransferToken 转账其他代币
func WemixTransferToken(privateKeys string, amount *big.Int, toAddress string, tokenAddress string, MaxNonce uint64) (transferData interface{}, currentNonce uint64, err error) {
	cache := service.Cache.New()
	rpcUrl := gconv.String(cache.Get("wemix_rpc_url"))
	client, err := ethclient.Dial(rpcUrl)
	if err != nil {
