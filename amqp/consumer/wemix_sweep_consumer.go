...
type WemixSweepConsumer struct{}

// Consumer 实现币安扫块消费者
func (b *WemixSweepConsumer) Consumer(dataByte []byte, key uint64) error {
	transfer := rpc.WemixTransactions{}
	err := json.Unmarshal(dataByte, &transfer)
	if err != nil {
		return err
	} //json解析到结构体里面
	g.Log().File("wemix-consumer.{Y-m-d}.log").Printf("消费 %v", transfer.Hash)
	fromAddress := transfer.From
	//手续费地址
	wemixFeeAddress, err := service.SysConfig.GetConfigByKey("sys.wemixFeeAddress")
	if err != nil {
		return err
	}
	//提币地址
	wemixWithdrawAddress, err := service.SysConfig.GetConfigByKey("sys.wemixWithdrawAddress")
	if err != nil {
		return err
	}

	//判断一下这是合约充值还是获取地址充值
	if g.Config().GetBool("wemix.contract_recharge") && transfer.To == g.Config().GetString("wemix.contract_address") {
		err = wemixContractRechargeHandle(&transfer)
	} else {
		if g.Config().GetBool("wemix.address_recharge") {
			if fromAddress == strings.ToLower(wemixWithdrawAddress.ConfigValue) || fromAddress == strings.ToLower(wemixFeeAddress.ConfigValue) {
				return nil
			}
			err = WemixAddressRechargeHandle(&transfer)
		}
	}
	return err
}

func wemixContractRechargeHandle(transfer *rpc.WemixTransactions) (err error) {
	//检查交易状态
	...
	fromAddress := transfer.From
	ctx := gctx.New()
	coinAddress, err := sservice.Currency.GetWemixCoinAddress(ctx)
	if err != nil {
		return err
	}
	...
	if exists == nil {
		data1 := dao.RechargeAddReq{MainChain: "wemix", BlockHash: transfer.BlockHash, CoinToken: coinToken, CoinToken1: coinToken1, FromAddress: fromAddress, ToAddress: toAddress, Amount: amount, Amount1: amount1, ContractAddress: contractAddress,
		...
		wemixSendNotify(&data1)
	}
	return nil
}

func WemixAddressRechargeHandle(transfer *rpc.WemixTransactions) (err error) {
	ctx := gctx.New()
	userAddress, err := sservice.Address.GetWemixAllAddress(ctx)
	if err != nil {
		return err
	}
	coinAddress, err := sservice.Currency.GetWemixCoinAddress(ctx)
	...
	if exists == nil {
		data1 := dao.RechargeAddReq{MainChain: "wemix", BlockHash: transfer.BlockHash, CoinToken: coinToken, FromAddress: fromAddress, ToAddress: toAddress,
...
func wemixSendNotify(recharge *dao.RechargeAddReq) {
	//检测是否配置了充值回调地址
