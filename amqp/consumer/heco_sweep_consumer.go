...
type HecoSweepConsumer struct{}

// Consumer 实现币安扫块消费者
func (b *HecoSweepConsumer) Consumer(dataByte []byte, key uint64) error {
	transfer := rpc.HecoTransactions{}
	err := json.Unmarshal(dataByte, &transfer)
	if err != nil {
		return err
	} //json解析到结构体里面
	g.Log().File("heco-consumer.{Y-m-d}.log").Printf("消费 %v", transfer.Hash)
	fromAddress := transfer.From
	//手续费地址
	bnbFeeAddress, err := service.SysConfig.GetConfigByKey("sys.hecoFeeAddress")
	if err != nil {
		return err
	}
	//提币地址
	bnbWithdrawAddress, err := service.SysConfig.GetConfigByKey("sys.hecoWithdrawAddress")
	if err != nil {
		return err
	}

	//判断一下这是合约充值还是获取地址充值
	if g.Config().GetBool("heco.contract_recharge") && transfer.To == g.Config().GetString("heco.contract_address") {
		err = heconContractRechargeHandle(&transfer)
	} else {
		if g.Config().GetBool("heco.address_recharge") {
			if fromAddress == strings.ToLower(bnbWithdrawAddress.ConfigValue) || fromAddress == strings.ToLower(bnbFeeAddress.ConfigValue) {
				return nil
			}
			err = HecoAddressRechargeHandle(&transfer)
		}
	}
	return err
}

func heconContractRechargeHandle(transfer *rpc.HecoTransactions) (err error) {
	//检查交易状态
	...
	fromAddress := transfer.From
	ctx := gctx.New()
	coinAddress, err := sservice.Currency.GetHecoCoinAddress(ctx)
	if err != nil {
		return err
	}
	...
	if exists == nil {
		data1 := dao.RechargeAddReq{MainChain: "heco", BlockHash: transfer.BlockHash, CoinToken: coinToken, CoinToken1: coinToken1, FromAddress: fromAddress, ToAddress: toAddress, Amount: amount, Amount1: amount1, ContractAddress: contractAddress,
		...
		hecoSendNotify(&data1)
	}
	return nil
}

func HecoAddressRechargeHandle(transfer *rpc.HecoTransactions) (err error) {
	ctx := gctx.New()
	userAddress, err := sservice.Address.GetHecoAllAddress(ctx)
	if err != nil {
		return err
	}
	coinAddress, err := sservice.Currency.GetHecoCoinAddress(ctx)
	...
	if exists == nil {
		data1 := dao.RechargeAddReq{MainChain: "heco", BlockHash: transfer.BlockHash, CoinToken: coinToken, FromAddress: fromAddress, ToAddress: toAddress,
...
func hecoSendNotify(recharge *dao.RechargeAddReq) {
	//检测是否配置了充值回调地址
