...
type bscWithdraw struct {
	SystemBase
}

var BscWithdraw = new(bscWithdraw)

// List 列表
func (c *bscWithdraw) List(r *ghttp.Request) {
...
// Get 获取
func (c *bscWithdraw) Get(r *ghttp.Request) {
	id := r.GetUint64("id")
	if id == 0 {
...
// AuditSuccess 审核通过
func (c *bscWithdraw) AuditSuccess(r *ghttp.Request) {
	ids := r.GetArray("ids")
	if len(ids) <= 0 {
		c.FailJsonExit(r, "参数错误")
	}
...
// AuditFail 审核拒绝
func (c *bscWithdraw) AuditFail(r *ghttp.Request) {
	ids := r.GetArray("ids")
	if len(ids) <= 0 {
		c.FailJsonExit(r, "参数错误")
	}
...
// Withdraw 手动提现请求
func (c *bscWithdraw) Withdraw(r *ghttp.Request) {
	id := r.GetInt64("id")
	...
	if err != nil || (info.Status == 5 || info.Status == 6) {
		c.FailJsonExit(r, "参数错误"+err.Error())
	}
...
// Callback 发起回调
func (c *bscWithdraw) Callback(r *ghttp.Request) {
	ids := r.GetArray("ids")
	if len(ids) <= 0 {
		c.FailJsonExit(r, "参数错误")
	}
	...
	for _, value := range list {
		...
		if value.NotifyUrl != "" {
			data = url.Values{
				...
				"remarks":          {value.Remarks},
				"hash":             {value.Hash},
				"token_id":         {gconv.String(value.TokenId)},
				"url":              {value.Url},
			}
			resp, _ := g.Client().PostForm(value.NotifyUrl, data)
			g.Log().File("callback.{Y-m-d}.log").Printf("发送提现回调请求 请求域名:【%v】 请求参数:【%v】 返回code码【%v】", value.NotifyUrl, data.Encode(), resp.StatusCode)
