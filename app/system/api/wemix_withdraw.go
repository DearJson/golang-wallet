...
type wemixWithdraw struct {
	SystemBase
}

var WemixWithdraw = new(wemixWithdraw)

// List 列表
func (c *wemixWithdraw) List(r *ghttp.Request) {
...
// Get 获取
func (c *wemixWithdraw) Get(r *ghttp.Request) {
	id := r.GetUint64("id")
	if id == 0 {
...
// AuditSuccess 审核通过
func (c *wemixWithdraw) AuditSuccess(r *ghttp.Request) {
	ids := r.GetArray("ids")
	if len(ids) <= 0 {
		c.FailJsonExit(r, "参数错误")
	}
...
// AuditFail 审核拒绝
func (c *wemixWithdraw) AuditFail(r *ghttp.Request) {
	ids := r.GetArray("ids")
	if len(ids) <= 0 {
		c.FailJsonExit(r, "参数错误")
	}
	...
	for _, value := range list {
		...
		if value.NotifyUrl != "" {
			...
			g.Log().File("callback.{Y-m-d}.log").Printf("发送充值回调请求 请求域名:【%v】 请求参数:【%v】 返回code码【%v】", value.NotifyUrl, data.Encode(), resp.StatusCode)
			resp.Body.Close()
		}
	}
	c.SusJsonExit(r)
...
// Withdraw 手动提现请求
func (c *wemixWithdraw) Withdraw(r *ghttp.Request) {
	id := r.GetInt64("id")
...
// Callback 发起回调
func (c *wemixWithdraw) Callback(r *ghttp.Request) {
	ids := r.GetArray("ids")
	if len(ids) <= 0 {
		c.FailJsonExit(r, "参数错误")
	}
