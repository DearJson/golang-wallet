...
type hecoWithdraw struct {
	SystemBase
}

var HecoWithdraw = new(hecoWithdraw)

// List 列表
func (c *hecoWithdraw) List(r *ghttp.Request) {
...
// Get 获取
func (c *hecoWithdraw) Get(r *ghttp.Request) {
	id := r.GetUint64("id")
	if id == 0 {
...
// AuditSuccess 审核通过
func (c *hecoWithdraw) AuditSuccess(r *ghttp.Request) {
	ids := r.GetArray("ids")
	if len(ids) <= 0 {
		c.FailJsonExit(r, "参数错误")
	}
...
// AuditFail 审核拒绝
func (c *hecoWithdraw) AuditFail(r *ghttp.Request) {
	ids := r.GetArray("ids")
	if len(ids) <= 0 {
		c.FailJsonExit(r, "参数错误")
	}
...
// Withdraw 手动提现请求
func (c *hecoWithdraw) Withdraw(r *ghttp.Request) {
	id := r.GetInt64("id")
...
// Callback 发起回调
func (c *hecoWithdraw) Callback(r *ghttp.Request) {
	ids := r.GetArray("ids")
	if len(ids) <= 0 {
		c.FailJsonExit(r, "参数错误")
	}
