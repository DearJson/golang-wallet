...
func (c *auth) login(r *ghttp.Request) (string, interface{}) {
	...
	//判断谷歌验证码是否正确
	if g.Config().GetBool("googleAuth.open") {
		res, _ := library.NewGoogleAuth().VerifyCode(g.Config().GetString("googleAuth.secret"), apiReq.GoogleCode)
		if !res {
			c.FailJson(true, r, "谷歌验证码输入错误")
		}
	}
