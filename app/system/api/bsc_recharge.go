...
var BscRecharge = new(bscRecharge)

// List 列表
func (c *bscRecharge) List(r *ghttp.Request) {
...
// Get 获取
func (c *bscRecharge) Get(r *ghttp.Request) {
	id := r.GetInt64("id")
...
// Callback 发起回调
func (c *bscRecharge) Callback(r *ghttp.Request) {
	ids := r.GetArray("ids")
	if len(ids) <= 0 {
	...
	for _, value := range list {
		...
		data = url.Values{
			...
			"customeCoin":       {value.CustomeCoin},
			"customeAmount":     {value.CustomeAmount},
		}
